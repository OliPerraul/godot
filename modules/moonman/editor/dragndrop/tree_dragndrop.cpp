...
#include "tree_dragndrop.h"

void MoMaTreeDragnDrop::_add_node_hierarchy(Node* p_node, TreeItem* p_item)
{
	if (!p_node)
	{
		return;
...
void MoMaTreeDragnDrop::update_tree()
{
	_tree_node->clear();
...
void MoMaTreeDragnDrop::_add_node(Node* p_node, TreeItem* p_parent_item, TreeItem*& r_item)
{
...
void MoMaTreeDragnDrop::setup(Variant p_tree, Variant p_node)
{
...
void MoMaTreeDragnDrop::_bind_methods()
{
	ClassDB::bind_method(D_METHOD("setup", "tree_node", "root_node"), &MoMaTreeDragnDrop::setup);
	ClassDB::bind_method(D_METHOD("update_tree"), &MoMaTreeDragnDrop::update_tree);
}
